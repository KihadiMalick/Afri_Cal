...
//   emerald-owl.png, mosquito.png, diamond-simba.png
//
  ...
  return (
    <GestureDetector gesture={gesture}>
      <Animated.View style={[{ position:'absolute', alignSelf:'center', zIndex:isTop?10:5 }, isTop?animStyle:behindStyle]}>
        {/* Cadre métallique biseauté */}
        <View style={{
          width:CHAR_CARD_W, borderRadius:24, padding:4, borderWidth:2,
          borderTopColor:char.borderColors[0]+'CC', borderLeftColor:char.borderColors[1]+'99',
          borderRightColor:char.borderColors[2]+'99', borderBottomColor:char.borderColors[2]+'66',
          backgroundColor:char.borderColors[2]+'30',
          shadowColor:char.borderColors[0], shadowOffset:{width:0,height:4}, shadowOpacity:0.25, shadowRadius:16, elevation:12,
        }}>
          {/* Liseré intérieur */}
          <View style={{ borderRadius:20, borderWidth:1.5, borderColor:char.borderColors[0]+'35', overflow:'hidden' }}>
            <View style={{ backgroundColor:'#151B23', borderRadius:18 }}>
              <CircuitPattern width={CHAR_CARD_W-12} height={CHAR_CARD_H} color={char.borderColors[0]+'08'} />

              {/* Image personnage */}
              <View style={{ height:220, backgroundColor:'#0A0E14', borderTopLeftRadius:18, borderTopRightRadius:18, overflow:'hidden' }}>
                {char.image ? (
                  <Image source={char.image} style={{ width:'100%', height:'100%' }} resizeMode="cover" />
                ) : (
                  <View style={{ flex:1, alignItems:'center', justifyContent:'center', backgroundColor:char.borderColors[2]+'15' }}>
                    <Text style={{fontSize:80}}>{char.fallbackEmoji}</Text>
                  </View>
                )}
                {/* Badge tier */}
              ...
              {/* Infos bas */}
              <View style={{ padding:14, borderTopWidth:1, borderTopColor:char.borderColors[0]+'15' }}>
                <Text style={{ color:'#EAEEF3', fontSize:18, fontWeight:'800', letterSpacing:2, marginBottom:4 }}>{char.name}</Text>
                <Text style={{ color:char.levelColor, fontSize:11, fontWeight:'600' }}>{char.power}</Text>
              </View>
            </View>
          </View>
        </View>
  ...
  var characters = [
    {
      ...
      image: require('./assets/emerald-owl.png'),
      fallbackEmoji: '\uD83E\uDD89',
    },
    {
      ...
      image: null, // require('./assets/mosquito.png')
      fallbackEmoji: '\uD83E\uDD9F',
    },
    {
      ...
      image: null, // require('./assets/diamond-simba.png')
      fallbackEmoji: '\uD83E\uDD81',
    },
  ];
      ...
      fetch(SUPABASE_URL+'/rest/v1/rpc/create_user_profile',{
        ...
        showAlert(lang==='fr'?'Bienvenue sur LIXUM !':'Welcome to LIXUM!',
          ...
          [{text:lang==='fr'?'Commencer':'Get started',color:'#00D984',onPress:function(){
            console.log('Register complete, LixTag:', finalTag, 'User:', userId);
        ...
        showAlert(lang==='fr'?'Bienvenue sur LIXUM !':'Welcome to LIXUM!',
          ...
          [{text:lang==='fr'?'Commencer':'Get started',color:'#00D984',onPress:function(){
            console.log('Register partial, LixTag:', clientLixTag);
