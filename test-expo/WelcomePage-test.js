  ...
  return (
    <View style={{ alignItems: 'center', marginBottom: 10 }}>
      <Svg width={w} height={h} viewBox={'0 0 ' + w + ' ' + h}>
        ...
        <Circle cx={leftX} cy={centerY} r={reactorR - 7} fill="none" stroke="#FF8C42" strokeWidth={0.8} strokeDasharray="4 2" opacity={0.3} />
        <Circle cx={leftX} cy={centerY} r={11} fill="rgba(255,140,66,0.08)" />
        <Svg>
          <Path d={'M' + (leftX - 1) + ' ' + (centerY - 7) + 'L' + (leftX - 4) + ' ' + centerY + 'L' + (leftX + 3) + ' ' + centerY + 'L' + (leftX - 1) + ' ' + (centerY + 7) + 'Z'} fill="#FF8C42" opacity={0.7} />
        </Svg>
        {/* Satellite vert sur orbite gauche */}
        <Circle cx={leftX} cy={centerY - reactorR - 3} r={2.5} fill="#00D984" />
        <Circle cx={leftX} cy={centerY - reactorR - 3} r={4} fill="#00D984" opacity={0.2} />
        ...
        {Array.from({ length: 18 }, function(_, i) {
          ...
          return (
            <G key={'dna' + i}>
              ...
              <Circle cx={x1} cy={y} r={depth > 0 ? 2 : 1} fill="#00D984" opacity={depth > 0 ? 0.7 : 0.2} />
        ...
        <Circle cx={rightX} cy={centerY} r={reactorR - 7} fill="none" stroke="#4DA6FF" strokeWidth={0.8} strokeDasharray="4 2" opacity={0.3} />
        <Circle cx={rightX} cy={centerY} r={11} fill="rgba(77,166,255,0.08)" />
        <Svg>
          <Path d={'M' + (rightX - 1) + ' ' + (centerY - 7) + 'L' + (rightX - 4) + ' ' + centerY + 'L' + (rightX + 3) + ' ' + centerY + 'L' + (rightX - 1) + ' ' + (centerY + 7) + 'Z'} fill="#4DA6FF" opacity={0.7} />
        </Svg>
      ...
      <View style={{ position: 'absolute', top: labelY - 5, left: 0, width: w, height: 14 }}>
        <Text style={{ position: 'absolute', left: leftX - 30, width: 60, textAlign: 'center', color: '#FF8C42', fontSize: 7, fontWeight: '700', letterSpacing: 0.6 }}>{labelLeft}</Text>
        <Text style={{ position: 'absolute', left: centerX - 24, width: 48, textAlign: 'center', color: '#00D984', fontSize: 7, fontWeight: '800', letterSpacing: 0.6 }}>{labelCenter}</Text>
        <Text style={{ position: 'absolute', left: rightX - 30, width: 60, textAlign: 'center', color: '#4DA6FF', fontSize: 7, fontWeight: '700', letterSpacing: 0.6 }}>{labelRight}</Text>
      </View>
    </View>
  );
}
                    ...
                    {slide.badges.map(function (badge, i) {
                      return (
                        <View key={i} style={{
                          ...
                          <Text style={{ color: '#00D984', fontSize: 8, fontWeight: '700', letterSpacing: 0.8 }}>{badge}</Text>
                        </View>
                      );
                    })}
                  </View>
    ...
    <TouchableOpacity activeOpacity={0.7} onPress={onPress} style={{ width: '100%', borderRadius: 14 }}>
      <View style={{
        ...
        >
          ...
          <Text style={{ color: '#0D1117', fontSize: 16, fontWeight: '800', letterSpacing: 1.5 }}>{text}</Text>
        </LinearGradient>
      </View>
    </TouchableOpacity>
  );
  ...
  var slides = [
    {
      ...
      lines: t.slide1Lines, badges: ['AI-POWERED', 'REAL-TIME', '524 PLATS'], color: '#00D984',
    },
    {
    ...
    },
    {
      ...
      lines: t.slide3Lines, badges: ['SCORE 0-100', 'BHI', 'AI COACH'], color: '#00D984',
    },
  ];
            ...
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
              {!allSwiped ? (
                ...
                <Animated.View entering={FadeInDown.duration(600).springify()}>
                  ...
                  <View style={{
                    width: CARD_W + 8, height: CARD_H + 8, borderRadius: 24, padding: 4,
                    borderWidth: 2, borderTopColor: '#D4AF37', borderLeftColor: '#C5A028',
