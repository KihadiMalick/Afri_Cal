...
const RepasPage = ({ onNavigate }) => {
  ...
  // Placeholder — sera complété en prompt 5/5 avec l'appel API
  var generateAlixenProposals = async function(category) {
    ...
    try {
      ...
      // Construire la liste des ingrédients si mode "mes ingrédients"
      var userIngList = '';
      ...
      // Construire les repas du jour en texte
      ...
      try {
        ...
        if (proposals.length > 0) {
          ...
          // Générer un conseil si les calories restantes sont > 800 la nuit
          if (ctx.timeOfDay === 'night' && ctx.remaining > 800) {
            setAlixenAdvice(
              ctx.userName + ', il te reste ' + Math.round(ctx.remaining) + ' kcal mais il est ' + ctx.hour + 'h. ' +
              'C\'est trop pour un seul repas nocturne. Je te suggère un repas de 400-500 kcal ce soir et on ajuste demain au petit-déjeuner.'
        ...
        // Fallback : propositions statiques basées sur le contexte
    ...
    try {
      const response = await fetch(
        'https://yuhordnzfpcswztujovi.supabase.co/functions/v1/lookup-barcode',
        {
          ...
          headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inl1aG9yZG56ZnBjc3d6dHVqb3ZpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzgzMzk0MzksImV4cCI6MjA1MzkxNTQzOX0.H2mCpGz_RGvMR8TBtX1QLyEJHFVKpXHcNMEf5vGn13M',
          },
      ...
      if (result.product) {
        // Filtrer les produits non-alimentaires
        ...
        // PRODUIT NON TROUVÉ → proposer le mode photo
              ...
              >
                ...
                {alixenRecipeScreen === 'welcome' && (
                  <View style={{ paddingHorizontal: wp(16), paddingTop: wp(14) }}>
                    ...
                    {/* Boutons catégories — grille 3 colonnes */}
                    <Text style={{
                    ...
                    <View style={{
                      ...
                      {ALIXEN_CATEGORIES.map(function(cat) {
                        // La nuit, mettre en avant les catégories légères
                ...
                {alixenRecipeScreen === 'detail' && alixenSelectedRecipe && (
                  <View style={{ paddingHorizontal: wp(16), paddingTop: wp(14) }}>
                    ...
                    {/* Liste ingrédients avec grammes précis */}
                    <View style={{ marginBottom: wp(16) }}>
                    ...
                    {/* Conseil ALIXEN (nocturne ou dépassement) */}
                    {alixenAdvice && (
                      <View style={{
                        backgroundColor: 'rgba(255,140,66,0.06)',
                    ...
                    <View style={{ gap: wp(10), marginBottom: wp(40) }}>
                      ...
                      <Pressable
                        onPress={function() {
                          // Déterminer le créneau automatiquement
                          var hour = new Date().getHours();
                          var slot = hour < 10 ? 'breakfast' : hour < 14 ? 'lunch' : hour < 21 ? 'dinner' : 'snack';

                          // Vérifier si c'est un repas lourd la nuit
          ...
          <View style={{ flex: 1, backgroundColor: '#0D1117' }}>
            ...
            <View style={{
              ...
              <Pressable onPress={function() {
                var hasActiveTimer = Object.keys(cookingTimers).some(function(k) { return cookingTimers[k].running; });
                if (hasActiveTimer) {
                  Alert.alert('Minuteurs en cours', 'Tu as des minuteurs actifs. Quitter ?', [
                    { text: 'Continuer', style: 'cancel' },
                    { text: 'Quitter', style: 'destructive', onPress: function() { setCookingTimers({}); setShowCookingMode(false); }},
                  ]);
                } else {
                  setShowCookingMode(false);
                }
              ...
              <View style={{
                ...
                <View style={{
                  ...
                  <Text style={{ color: '#EAEEF3', fontSize: fp(15), textAlign: 'center', lineHeight: fp(22), marginBottom: wp(20) }}>
                    {alixenContext ? alixenContext.userName : 'Chef'}, vérifie {cookingAlarm.label ? 'tes ' + cookingAlarm.label.toLowerCase() : 'ta cuisson'} — le temps est écoulé !
                  </Text>
                  <Pressable
                    onPress={function() { dismissAlarm(); }}
                    style={function(state) {
