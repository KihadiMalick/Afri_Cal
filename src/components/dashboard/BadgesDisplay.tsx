        ...
        {badges.map((badge) => (
          <div
            ...
            className={`rounded-2xl p-3 text-center transition-all duration-300 ${
              badge.unlocked
                ? "bg-accent-500/10 border border-accent-500/30 shadow-glow-accent"
                : "bg-dark-700 border border-dark-600 opacity-50"
            }`}
          >
            <span className="text-2xl block mb-1">{badge.icon}</span>
            <p className={`text-xs font-semibold ${badge.unlocked ? "text-accent-400" : "text-dark-300"}`}>
              {locale === "fr" ? badge.nameFr : badge.name}
            </p>
            <p className="text-[10px] text-dark-200 mt-0.5">
